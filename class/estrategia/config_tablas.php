<?php
    class configTablas
    {
        ...
        */ 
        ...
        /*
            *** Lista todos los campos configurados de una tabla de un cedente en particular
        */ 

        public function getFiltrar_camposCedente($idTabla,$cedente){
            $db = new Db();
            $TablasArray = array();
            $SqlTablas = "select * from SIS_Columnas_Estrategias where id_tabla = '$idTabla' and FIND_IN_SET('$cedente',Id_Cedente) order by columna desc";
            $Tablas = $db -> select($SqlTablas);
            foreach($Tablas as $Tabla){
                $Array = array();
                $Array['columna'] = utf8_encode($Tabla["columna"]);
        ...
        */
        ...
        function getNombreTabla($Tabla){
            $db = new Db(); 
            $Sql = "select nombre from SIS_Tablas where id='".$Tabla."'";
            $Nombre = $db -> select($Sql);
            return $Nombre[0]["nombre"];
        }
        function getNombreCampo($Campo){
            $db = new Db(); 
            $Sql = "select columna from SIS_Columnas_Estrategias where id='".$Campo."'";
            $Nombre = $db -> select($Sql);
            return $Nombre[0]["columna"];
        }
        function getIdTablaByNombre($Tabla){
            $db = new Db(); 
            $Sql = "select id from SIS_Tablas where nombre='".$Tabla."'";
            $ID = $db -> select($Sql);
            return $ID[0]["id"];
        }
        function getIdCampoByNombreAndTabla($Tabla,$Campo){
            $db = new Db(); 
            $Sql = "select id from SIS_Columnas_Estrategias where columna='".$Campo."' and id_tabla='".$Tabla."'";
            $ID = $db -> select($Sql);
            return $ID[0]["id"];
        }
    }
