...
class configCampos
{
    ...
    public function insertarCamposConfig($campos, $idtabla)
    {
        ...
        foreach ($campos as $campo) {
            ...
            $idCampo = $campo[5];
            if ($idCampo == "") { // no esta en bd
                $SqlInsertCampo = "insert into SIS_Columnas_Estrategias (columna, id_tabla, tipo, tipo_dato, orden, logica, Cedente, Id_Cedente) values('" . $columna . "', '" . $id_tabla . "', '" . $tipo . "','" . $tipo_dato . "','" . $orden . "','" . $logica . "','" . $cedente . "','" . $_SESSION["cedente"] . "')";
                $InsertCampo    = $db->query($SqlInsertCampo);
            } else {
                $SqlUpdateCampo = "update SIS_Columnas_Estrategias set columna = '" . $columna . "', tipo = '" . $tipo . "', tipo_dato = '" . $tipo_dato . "', orden = '" . $orden . "', logica = '" . $logica . "', Cedente = '" . $cedente . "', Id_Cedente=CONCAT(Id_Cedente,',".$cedente."') where id='" . $idCampo . "' ";
    ...
    function getListar_camposNoConfig($nombreTabla, $camposArray)
    {
        ...
        $SqlCampos   = "SELECT * FROM information_schema.columns WHERE table_name = '".$nombreTabla."'";
        $camposTabla = $db->select($SqlCampos);

        //if ((count($camposTabla) != count($camposArray)) && $camposArray) {
            $i = 0;
