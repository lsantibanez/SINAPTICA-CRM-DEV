...
$(document).ready(function() {
    ...
    if(Parametros != ""){
        ...
        for (var i = 0; i < ParametrosArray.length; i++) {
            ...
            switch (ParametroArray[0]) {
                case "e":
                    $('select[name=Filtros]').val("");
                    $('select[name=Filtros]').find('option[value="Evaluacion"]').attr("selected", true);
                    $("#EvaluationFilterContainer").hide();
                    $("#FullFilterContainer").hide();
                    UpdateRecordTable([]);
                    $("#EvaluationFilterContainer").show();
                    $("input[name='idEvaluacion']").val(ParametroArray[1]);
                    BuscarEvaluacion(ParametroArray[1]);
                    break;
            }
        }
    }
    ...
    $("select[name='Filtros']").change(function(){
        ...
        switch(Filtros){
            ...
            break;
            case "Todos":
                $("#FullFilterContainer").show();
            break;
        }
    });
    $("#BuscarEvaluacion").click(function(){
        var idEvaluacion = $("input[name='idEvaluacion']").val();
        ...
        $.ajax({
            ...
            success: function(data){
                if(isJson(data)){
                    ToReturn = JSON.parse(data);
                    if(ToReturn.result){
                        FocoSocket.emit('showCalidadNotifications', { idGrabacion: RecordId, idMandante: GlobalData.id_mandante })
                        UpdateTableObjeciones(RecordId);
                    }
                }
            },
    ...
    function BuscarEvaluacion(idEvaluacion){
        $.ajax({
            type: "POST",
            url: "../includes/calidad/buscarGrabacionEvaluacion.php",
            data: {
            ...
            success: function(data){
                if(isJson(data)){
                    ...
                    Ejecutivo[1] = data.Personal.nombreUsuario;
                    Ejecutivo[0] = data.Personal.nombrePersonal;
                    UpdateRecordTable(dataSet);
                }
            },
            error: function(){
