$(document).ready(function()
{
  ...
  $.ajax({
      ...
      data: datos,
			beforeSend: function () {
				$('.listadoEstretegias').html('Cargando...');
			},
  ...
  function listaEstrategiasInactivas() {
		var cedente = $('#cedente').val();
		var nombreUsuario = $('#nombreUsuario').val();
		var datos = "cedente="+cedente+"&nombreUsuario="+nombreUsuario;
		$.ajax({
			type:"POST",
			url:"../includes/estrategia/listEstrategiaInactiva.php",
			data:datos,
			success:function(data) {
				$('.lisEstretegiasInactivas').html(data);
				$('#TablaVerEstrategiaIna').dataTable();
			}
		});
  }

  $("body").on("click",".estrategiasInactivas", function(){
        bootbox.dialog({
            title: "Estrategias Inactivas",
            message: $("#listadoEstretegiasInactivas").html(),   

            size: 'large'
       }).off("shown.bs.modal");
	   listaEstrategiasInactivas();     
        
    });

  $(document).on('click','.eliminar',function(){
    var id = $(this).attr("id");
	var data = "idEstrategia="+id;
	
    bootbox.confirm("¿Esta seguro que desea eliminar la estrategia y todas sus tablas dependientes?", function(result) {
    if (result) {
      $.ajax({
       type:"POST",
       url:"../includes/estrategia/eliminarEstrategia.php",
       data:data,
       success:function(data) {
        $('#'+id+'').remove();
        bootbox.alert("Estrategia eliminada con éxito");
       }
      });
    }
    });
  ...
  $(document).on('click','.desactivar',function(){
    ...
    bootbox.confirm("¿Esta seguro que desea desactivar la estrategia?", function(result) {
    if (result) {
      $.ajax({
       type:"POST",
       url:"../includes/estrategia/desactivarEstrategia.php",
       data:data,
       success:function(data) {
        $('#'+id+'').remove();
        bootbox.alert("Estrategia desactivada con éxito");
       }
      });
    }
    });
  ...
  $(document).on('click','.activar',function(){
    ...
    bootbox.confirm("¿Esta seguro que desea activar la estrategia?", function(result) {
    if (result) {
      $.ajax({
       type:"POST",
       url:"../includes/estrategia/activarEstrategias.php",
       data:data,
       success:function(data) {
		bootbox.alert("Estrategia activada con éxito");   
        $('#'+id+'').remove();		
		location.reload();
       }
      });
  ...
  $(document).on('click', '.VerEstrategia', function()
  {
 	...
 	$('#crear_estrategia').submit(function(e)
    {
        ...
        var idUsuario = $('#idUsuario').val();
	        ...
	        //window.location.replace("ver_estrategias.php?id_estrategia="+id_estrategia);
					...
					window.location.replace("segmentacion.php?id_estrategia="+id_estrategia);
				}
			});
   		}
    });
	...
	$("body").on('change', '.nombreEstrategia', function () {
		var ObjectMe = $(this);
		var ObjectTR = ObjectMe.closest("tr");
		var idEstrategia = ObjectTR.attr("id");
		var nombreEstrategia = ObjectMe.val();
		$.ajax({
			type: "POST",
			url: "../includes/estrategia/cambiarNombreEstrategia.php",
			data: {
				...
				if(isJson(data)){
					...
					if(data.result){
						$.niftyNoty({
							type: 'success',
							icon: 'fa fa-close',
							message: "Nombre de Estrategia actualizada satisfactoriamente.",
							container: 'floating',
							timer: 5000
						});
					}
