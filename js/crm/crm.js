$(document).ready(function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response == "") {
                    ...
                    $("#script_cobranza_mostrar").html(
                        "Buenos días/Tardes, necesito comunicarme con el encargado de pagos a proveedores"
                    );
                } else {
                    $("#script_cobranza_ocultar").hide();
    ...
    if (id_dial == 1) {
                ...
                success: function (response) {
                    ...
                    if (isJson(response)) {
                        ...
                        if (json.result) {
                            ...
                            $("#mostrar_deudas").prepend(
                                '<button id="crearConvenio" class="btn btn-success " style="margin-bottom:20px">Crear convenio</button>'
                            );
                        }
                    }
            ...
            if ($("#ValorCuotas").val() != "" && $("#ValorCuotas").val() != "") {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        $(".body_convenio").prepend(
                            '<button id="excelConvenio" data-id="' +
                                last +
                                '" class="btn btn-success " style="margin-bottom:20px">Generar Excel</button>'
                        );
                        ...
                        $("#tablaCuotas").DataTable({
                            data: response,
                            columns: [{ data: "cuota" }, { data: "fecha_vencimiento" }, { data: "valor" }],
                        });
                            ...
                            columnDefs: [
                                {
                                    ...
                                    render: function (data, type, row) {
                                        return (
                                            "<div style='text-align: center;'><i class='btn fa fa-trash btn-danger btn-icon icon-lg DeleteGestion' id='" +
                                            row.gestion +
                ...
                success: function (response) {
                    ...
                    if (isJson(response)) {
                        ...
                        GestionPagosTable = $("#mostrar_gestion_pagos_dt").DataTable({
                            data: dataSet,
                            columns: [{ data: "rut" }, { data: "fecha" }, { data: "monto" }, { data: "operacion" }],
                            language: {
        ...
        $(document).on("change", "#seleccione_nivel2", function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (tipo_gestion == 5) {
                        $("#seleccione_nivel3").html(
                            "<select class='selectpicker' id='seleccione_nivel3' name='seleccione_nivel3'><option value='0'>Seleccione</option><option value='0'>COMPROMISO</option></select>"
                        );
                    } else {
                        $(".nivel_3_mostrar").html(response);
                    }
        ...
        $(document).on("change", "#seleccione_nivel3", function () {
            ...
            if (bandera == 0) {
                ...
                var rut_ultimo = $("#rut_ultimo").val();
                var nivel4 =
                    "nivel_3=" + $("#seleccione_nivel3").val() + "&cortar_valor=" + cortar_valor + "&rut=" + rut_ultimo;
                $.ajax({
                    type: "POST",
        ...
        $(document).on("click", "#guardar", function () {
            ...
            //Agregado para el envio de file, no se modifica nada de lo anterior...
            var insertar1 = new FormData();
            insertar1.append("nivel1", nivel1);
            insertar1.append("nivel2", nivel2);
            insertar1.append("nivel3", nivel3);
            insertar1.append("comentario", comentario);
            insertar1.append("rut", rut_ultimo);
            insertar1.append("fono_discado", fono_discado);
            insertar1.append("tipo_gestion", tipo_gestion_final);
            insertar1.append("cedente", cedente);
            insertar1.append("usuario_foco", nombre_usuario_foco);
            insertar1.append("lista", numero_cola);
            insertar1.append("fecha_compromiso", fecha_compromiso);
            insertar1.append("monto_compromiso", monto_compromiso);
            insertar1.append("tiempoLlamada", tiempoLlamada);
            insertar1.append("NombreGrabacion", NombreGrabacion);
            insertar1.append("asignacion", asignacion);
            insertar1.append("origen", origen);
            insertar1.append("facturas", facturas);
            insertar1.append("fechaAgendamiento", fechaAgendamiento);
            insertar1.append("Hablar", Hablar);
            insertar1.append("UrlGrabacion", UrlGrabacion);
            insertar1.append("canales", canales);
            insertar1.append("prioridades", prioridades);
            insertar1.append("ArrayCampos", JSON.stringify(ArrayCampos));
            if ($("#file").length > 0) {
                insertar1.append("file", $("#file")[0].files[0]);
            } else {
                insertar1.append("file", "");
            }

            if ($("#Derivacion").length > 0) {
                insertar1.append("derivacion", $("#Derivacion").val());
            } else {
                insertar1.append("derivacion", "");
            }

            if ($("#Refacturar").length > 0) {
                insertar1.append("refacturar", $("#Refacturar").val());
            } else {
                insertar1.append("refacturar", "");
            }

            if ($("#Monto").length > 0) {
                insertar1.append("monto", $("#Monto").val());
            } else {
                insertar1.append("monto", "");
            }

            if ($("#monto_compromiso").length > 0) {
                insertar1.append("montoAgregado", $("#monto_compromiso").attr("data-amount")); //('amount'));
            } else {
                insertar1.append("montoAgregado", 0);
            }
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response == 2) {
                        ...
                        var msg =
                            "No se puede enviar Mail , Cedente no tiene Template Cargado en la Base de Datos , Consulte con el Administrador.";
                        $.niftyNoty({
                            type: "danger",
                            icon: "fa fa-close",
                            message: msg,
        ...
        $(document).on("change", ".correo_cambiar", function () {
            ...
            console.log(cargo);
            var idmail =
                "id_mail=" + id_mail + "&mail=" + mail + "&nombre=" + nombre + "&cargo=" + cargo + "&obs=" + obs;
            $.ajax({
                type: "POST",
                    ...
                    success: function (response) {
                        if (isJson) {
                            ...
                            if (Json.result) {
                                ...
                                $("#tablaTelefonos tr." + id_reg)
                                    .find(".telefono")
                                    .val(Telefono);
                                $("#tablaDetalleTelefono tr." + id_reg)
                                    .find(".telefono")
                                    .val(Telefono);
                            }
                        }
        ...
        $(document).on("click", "#buscar_rut", function () {
            ...
            $.ajax({
                ...
                data: "prefijo=" + rut_buscado + "&estrategia=" + 0 + "&orden=" + 0 + "&tipo=" + 3,
                dataType: "json",
                success: function (response) {
                    $("#Cargando").modal("hide");
                    if (response) {
                        ...
                        $("#script_cobranza_mostrar").show();
                        mostrarNombreCliente(rut_buscado);
    ...
    $(document).on("click", ".Llamar", function () {
        ...
        if (CanCall) {
            ...
            var data =
                ...
                DialProvider +
                "&CodigoFoco=" +
                GlobalData.focoConfig.CodigoFoco +
                "&FonoPrefix=" +
                GlobalData.FonoPrefix.Prefix;
            console.log(data);
                        ...
                        success: function (response) {
                            ...
                            if (isJson(response)) {
                                ...
                                if (response.uno) {
                                    ...
                                    message =
                                        "No se encuentra registrado en el Softphone, consulte con el administrador";
                                    typeMessage = "danger";
                                    $(".Cortar").click();
                                }
                            }
    ...
    $(document).on("change", "select[name='NumeroTransferencia']", function () {
        ...
        if (Number != "") {
            if (CanalActivo != "") {
                if (DialProviderConfigured) {
                    DiscadorSocket.emit("transferCall", {
                        Canal: $("#CanalGrabacion").val(),
    ...
    if ($("#rut_buscado").val() != "") {
        /*var element = document.getElementById('conectar')
		element.click();*/
        var element = $("#seleccione_tipo_busqueda").val("2");
        element.change();
        var element = document.getElementById("buscar_rut");
        element.click();
    }

    $(document).on("change", "#facturas", function () {
        let facturas = $(this).val();
        let data = { facturas: facturas };
        $("#Cargando").modal("show");
        $.ajax({
            type: "POST",
            url: "../includes/crm/getInvoiceAmount.php",
            data: data,
            success: function (response) {
                $("#Cargando").modal("hide");
                ...
                $("#monto_compromiso").val(response.slice(0, -1));
                $("#monto_compromiso").attr("data-amount", response.slice(0, -1));
            },
        });
    });
});
